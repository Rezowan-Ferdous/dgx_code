...
class Encoder(nn.Module):
    def __init__(self, num_layers, r1, r2, num_f_maps, input_dim, num_classes, channel_masking_rate, att_type, alpha):
        ...
        self.PG= Prediction_Generation(num_layers,num_f_maps,input_dim,num_classes)
    ...
    def forward(self, x, mask):
        ...
        if self.channel_masking_rate > 0:
            ...
            x = self.dropout(x)
            x = x.squeeze(2)
        print(f" channel mask {x.shape}")
        feature = self.conv_1x1(x)
        feature_out= self.PG(x)
        feature= feature_out[:, :-1] # Ignore the last layer's output, for example, if the last dimension is 10
...
class MyTransformer(nn.Module):
    def __init__(self, num_decoders, num_layers, r1, r2, num_f_maps, input_dim, num_classes, channel_masking_rate,device):
        ...
        self.encoder_cls = Encoder(num_layers, r1, r2, num_f_maps, input_dim, num_classes, channel_masking_rate,
                               att_type='sliding_att', alpha=1)
        self.encoder_bound = Encoder(num_layers, r1, r2, num_f_maps, input_dim, 3, channel_masking_rate,
                                   att_type='sliding_att', alpha=1)
        self.decoders_cls = nn.ModuleList([copy.deepcopy(
        ...
        self.decoders_bound = nn.ModuleList([copy.deepcopy(
            Decoder(num_layers, r1, r2, num_f_maps, 3, 3, att_type='sliding_att',
                    alpha=exponential_descrease(s))) for s in range(num_decoders)])  # num_decoders

    def forward(self, x, mask):
        # Step 1: Reshape mask from (1, 1234) to (1, 1, 1234)
        mask = mask.unsqueeze(1)  # Shape becomes (1, 1, 1234)
        # Step 2: Expand the mask to match the required shape (1, 3, 1234)
        # This replicates the mask along the channel dimension
        cls_mask = mask.expand(-1, 8, -1).to(device)  # Shape becomes (1, 3, 1234)
        bound_mask= mask.expand(-1,3,-1).to(device)

        out, feature = self.encoder_cls(x, cls_mask)
        bound, featureb = self.encoder_bound(x, bound_mask)
        outputs = out.unsqueeze(0)
        bounds = bound.unsqueeze(0)

        for decoder in self.decoders_cls:
            out, feature = decoder(F.softmax(out, dim=1) * cls_mask[:, 0:1, :], feature * cls_mask[:, 0:1, :], mask)

            outputs = torch.cat((outputs, out.unsqueeze(0)), dim=0)

        for decoder in self.decoders_bound:
            bound, feature = decoder(F.softmax(bound, dim=1) * bound_mask[:, 0:1, :], feature * bound_mask[:, 0:1, :], mask)

            bounds = torch.cat((bounds, out.unsqueeze(0)), dim=0)
